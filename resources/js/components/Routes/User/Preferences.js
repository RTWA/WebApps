                    ...
                    <div className="px-4 py-5 bg-white sm:p-6 shadow sm:rounded-tl-md sm:rounded-tr-md">
                        <div className="grid grid-cols-6 gap-6">
                            <div className="col-span-6 sm:col-span-4 md:col-span-6 lg:col-span-4">
                                <label className="block font-medium text-sm text-gray-700" htmlFor="dark_mode">
                                    Dark Mode Option
                                </label>
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                                    <div onClick={() => setDarkMode('light')} className={`border-2 border-white dark:border-gray-800 w-full bg-white dark:bg-gray-800 rounded-xl shadow-xl overflow-hidden transform hover:-translate-y-2 cursor-pointer ${(preferences.dark_mode === 'light') ? `ring-4 ring-${UI.theme}-600 dark:ring-${UI.theme}-500 ring-opacity-50` : ''}`}>
                                        <div className="selector h-10 bg-gray-200 not-sr-only"></div>
                                        <div className="text-center">Light Mode Only</div>
                                    </div>
                                    <div onClick={() => setDarkMode('dark')} className={`border-2 border-white dark:border-gray-800 w-full bg-white dark:bg-gray-800 rounded-xl shadow-xl overflow-hidden transform hover:-translate-y-2 cursor-pointer ${(preferences.dark_mode === 'dark') ? `ring-4 ring-${UI.theme}-600 dark:ring-${UI.theme}-500 ring-opacity-50` : ''}`}>
                                        <div className="selector h-10 bg-gray-900 not-sr-only"></div>
                                        <div className="text-center">Dark Mode Only</div>
                                    </div>
                                    <div onClick={() => setDarkMode('')} className={`border-2 border-white dark:border-gray-800 w-full bg-white dark:bg-gray-800 rounded-xl shadow-xl overflow-hidden transform hover:-translate-y-2 cursor-pointer ${(preferences.dark_mode === '') ? `ring-4 ring-${UI.theme}-600 dark:ring-${UI.theme}-500 ring-opacity-50` : ''}`}>
